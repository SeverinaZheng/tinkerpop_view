...
public interface TraversalSource extends Cloneable, AutoCloseable {
    ...
    /** A map which maps: view_name-> ids, separated in type
     *  Key : View name ;
     *  Value : for node/edge: another map with Key: name of the return set, value is the set itself
     *          for path: another map with Key: name of the return set, value is the set of all the edge ids in a single path
     */
    public Map<String, Map<String,Collection<Long>>> nodeTable = new HashMap<>();
    public Map<String, Map<String,Collection<Long>>> edgeTable = new HashMap<>();
    public Map<String, Map<String,Collection<Collection<Long>>>> pathTable = new HashMap<>();

    /**
     * Add/Get a list of querying result into ViewMap in viewName
     */
    public default void addToViewMap(String ViewName, HashMap<String, Collection<Long>> returnVals){
        //returnVals will be in the form of "tgc,node"
        for (Map.Entry<String, Collection<Long>> entry :returnVals.entrySet()){
            String nameAndType = entry.getKey();
            String name = nameAndType.split(",")[0];
            String type = nameAndType.split(",")[1];
            Map<String,Collection<Long>> set;
            if(type.equals("node")){
                if(!nodeTable.containsKey(ViewName))
                    set= new HashMap<>();
                else
                    set = nodeTable.get(ViewName);
                set.put(name,entry.getValue());
                nodeTable.put(ViewName,set);
            }else if(type.equals("edge")){
                if(!edgeTable.containsKey(ViewName))
                    set= new HashMap<>();
                else
                    set = edgeTable.get(ViewName);
                set.put(name,entry.getValue());
                edgeTable.put(ViewName,set);
    ...
    public default Map<String,Collection<Long>> GetNodeViewByName(String ViewName){ return nodeTable.get(ViewName); }
